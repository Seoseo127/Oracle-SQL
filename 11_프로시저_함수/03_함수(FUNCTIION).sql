...
END;
**************************/
--BOOK 테이블에서 BOOKID로 책제목 확인 함수
--(파라미터값: BOOKID 값, RETURN 값: BOOKNAME)
create or replace FUNCTION GET_BOOKNAME (
    IN_ID IN NUMBER
) RETURN VARCHAR2 --(필수)리턴할 데이터 타입
AS
...
SELECT GET_BOOKNAME(2) FROM DUAL;
SELECT BOOKID, BOOKNAME FROM BOOK WHERE BOOKID = 2;
...
;
--============================
--(실습)고객ID 값을 받아서 고객명을 찾아주는 함수작성(CUSTOMER 테이블 사용)
--함수명: GET_CUSTNAME
