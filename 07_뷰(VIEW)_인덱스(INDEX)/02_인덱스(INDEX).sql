...
************************************/
--인덱스생성 : BOOK 테이블의 출판사(PUBLISHER) 컬럼에 IX_BOOK_PUB 인덱스 만들기
...
SELECT * FROM BOOK WHERE PRICE > 30000 AND PUBLISHER = '대한미디어'; --인덱스 사용
SELECT * FROM BOOK WHERE PRICE >= 20000;
SELECT * FROM BOOK ORDER BY PUBLISHER, PRICE;
-------------------------
--(주의) 인덱스가 있음에도 적용되지 않는 경우
-- LIKE 사용시 또는 데이터가 가공(변경)되는 경우
SELECT * FROM BOOK WHERE PUBLISHER LIKE '대한%'; --인덱스 적용(부분탐색)
...
SELECT * FROM BOOK WHERE SUBSTR(PUBLISHER, 1, 2) = '대한'; --적용안됨
SELECT * FROM BOOK WHERE PUBLISHER = 'Pearson'; --인덱스 적용
SELECT * FROM BOOK WHERE UPPER(PUBLISHER) = 'PEARSON'; --적용안됨
...
SELECT name FROM Customer WHERE name LIKE '박세리'; --F10 실행계획 확인
